...
void GFXCreateSwapChain(GFXAPI a_API, GFXSwapChainDescriptor *a_Descriptor, GFXSwapChainHandle *a_Handle)
{
	...
	HRESULT result = api->m_Factory->lpVtbl->CreateSwapChainForHwnd(api->m_Factory, (IUnknown*)commandQueue->m_CommandQueue, api->m_WindowHandle, &swapChainDesc, 0, 0, (IDXGISwapChain1**)&swapChain->m_SwapChain);
#if defined(_DEBUG)
	assert(S_OK == result);
#else
	GFX_UNUSED(result);
	...
	api->m_CurrentBackBufferIndex = swapChain->m_SwapChain->lpVtbl->GetCurrentBackBufferIndex(swapChain->m_SwapChain);
	*a_Handle = swapChain;
}
...
void GFXCreateCommandQueue(GFXAPI a_API, GFXCommandQueueDescriptor *a_Descriptor, GFXCommandQueueHandle *a_Handle)
{
	...
	HRESULT result = api->m_Device->lpVtbl->CreateCommandQueue(api->m_Device, &commandQueueDesc, &IID_ID3D12CommandQueue, (void**)&commandQueue->m_CommandQueue);
#if defined(_DEBUG)
	assert(S_OK == result);
#else
	GFX_UNUSED(result);
