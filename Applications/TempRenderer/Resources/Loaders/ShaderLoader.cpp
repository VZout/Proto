...
// #include "Resources/Helpers/LoadParameters.h"
...
ShaderLoader::ShaderLoader(const std::string &a_Extensions, Logger &a_Logger)
	: BaseLoader(a_Extensions, a_Logger)
{
...
ResourceData* ShaderLoader::Load(const LoadParameters &a_Parameters, Memory::MemoryPool &a_MemoryPool) const
{
	UNUSED(a_MemoryPool);
	UNUSED(a_Parameters);
